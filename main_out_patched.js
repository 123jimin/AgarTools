(function(window, $) {
	// Patch
	console.info("main_out.js patched for v402");
	var eatRatio = 0.81; // less than 0.89 for sure
	var myMinScore = 1e8;
    function onLoad() {
        getServerInfo();
    ...
    function computeScore() {
        ...
        for (var score = 0, i = 0; i < myBlobs.length; i++){
			...
			blobScore *= blobScore;
			if(myMinScore > blobScore) myMinScore = blobScore;
			score += blobScore;
        }
		return score;
    }
            ...
            draw: function() {
                if (this.shouldRender()) {
                    ...
                    if (noColor) {
						...
						if(ratio <= eatRatio/2) dispColor = "#C4FA2F";
						else if(ratio <= eatRatio) dispColor = "#11CF20";
						else if(ratio*eatRatio >= 2) dispColor = "#D60000";
						else if(ratio*eatRatio >= 1) dispColor = "#F28227";
						else dispColor = "#AAAAAA";
                        mainCtx.fillStyle = dispColor;
                        mainCtx.strokeStyle = dispColor;
                    } else {
					...
					// Patch: show other things' masses
